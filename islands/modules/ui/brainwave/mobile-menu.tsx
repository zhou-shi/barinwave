...
export const MenuItemLayout = ({ 
  ...
  <div className="flex items-center gap-3 w-full text-left">
    {Icon && 
      <div className="shrink-0 text-primary-100 mt-0.5">
    ...
    <div className="flex flex-col flex-1 gap-0.5">
      <span className={cn(
          ...
          isOpen ? "text-primary-100" : "text-primary-100/90"
      )}>
        {title}
      </span>
      
      {description && (
        <span className="text-sm text-muted-primary-100/90 line-clamp-2 leading-snug">
          {description}
        </span>
      )}
    </div>
    ...
    {hasChildren && (
        <ChevronDownIcon 
          className="size-5 text-muted-primary-100/90 shrink-0 transition-transform duration-300 ease-[cubic-bezier(0.87,0,0.13,1)]"
...
export const MobileMenuItem = ({ item, onLinkClick }: { item: NavigationMenuItems, onLinkClick: () => void }) => {
  ...
  if (!hasChildren) {
    return (
      <a 
        href={item.href || "#"}
        className="block py-3 px-4 text-lg font-medium text-primary-100/90 hover:bg-primary-50 rounded-md transition-colors"
        onClick={onLinkClick}
      >
        <MenuItemLayout 
          title={item.title} 
