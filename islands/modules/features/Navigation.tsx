...
import { MenuItemLayout, MobileMenuItem } from "../radix/mobile-menu";
import Brainwave from "./logos/brainwave";
import { ShineButton } from "../lightswind/ui/shine-button";
import { NavigationMenuItems } from "./header/types";
import { useState } from "preact/hooks";

const MotionSpan = ({className,...props}:HTMLMotionProps<"span">) => (
  <motion.span
    className={cn("bg-linear-to-r from-primary to-secondary w-7 h-1 rounded-full shadow-xs shadow-netral-950 inset-shadow-xs inset-shadow-netral-950",
      className
    )}
    {...props}
  />
...
export function Navigation({Menus=[]}: {Menus: NavigationMenuItems[]}) {
                ...
                <NavigationMenuItem key={index} >
                  {item.children && item.children.length > 0 ? (
                    <>
                      ...
                      <NavigationMenuContent>
                        <ul className="grid w-75 gap-4 max-h-full overflow-y-auto scrollbar-hide">
      ...
      {isMobile && (
        <>
          <motion.div
             ...
             className={cn(
               "absolute top-0 left-0 w-full h-28 z-40 xs:rounded-tl-2xl", // z-40 (di atas konten, di bawah tombol)
               "backdrop-blur-md", // Efek blur kaca
