...
} from "@/modules/shadcn/ui/navigation-menu";
import { 
   motion, 
   HTMLMotionProps, 
   MotionConfig, 
...
import { MenuItemLayout, MobileMenuItem } from "@/modules/radix/mobile-menu";
import { cn, keysToCamel, mapHugoMenuEntry } from "@/modules/lib/utils";
import { ToggleTheme } from "@/modules/lightswind/ui/theme-toggle";
import { IslandProps } from "@/modules/types";
import { ShineButton } from "@/modules/lightswind/ui/shine-button";
import { LogoDispatcher } from "../LogoDispatcher";
...
const ResponsiveNavigation = ({Menus=[], logoConfig}: {Menus: NavigationMenuItems[], logoConfig?: any}) => {
  ...
  return (
    ...
    >
      {isMobile && (
        <motion.div
          className="absolute inset-0 -z-10 bg-linear-to-br/oklch from-primary-600 to-secondary-600 backdrop-blur-xs"
      ...
      {isMobile && (
        <>
          ...
          <motion.div
             ...
             exit={{ opacity: 0 }}
             transition={{ duration: 0.3 }}
             className="absolute top-0 left-0 w-full z-50 flex items-center justify-between px-6 py-2 pointer-events-none"
          >
              {/* LOGO (Left) */}
              <div className="pointer-events-auto">
                <a href={logoConfig?.href || "#"}>
                   <LogoDispatcher config={logoConfig} />
                </a>
              </div>
...
const Brainwave = ({Menus=[], Params={}}: IslandProps) => {
   ...
   return (
      <header className="relative w-full py-2 rounded-lg flex items-center md:justify-between md:px-8 lg:px-16 bg-linear-to-r/oklch from-primary-600/5 to-secondary-600/5 z-50">
         {/* Logo */}
         <a href={logoConfig?.href || "#"}>
            <LogoDispatcher config={logoConfig}/>
         </a>
         <ResponsiveNavigation Menus={cleanMenus} logoConfig={logoConfig} />
